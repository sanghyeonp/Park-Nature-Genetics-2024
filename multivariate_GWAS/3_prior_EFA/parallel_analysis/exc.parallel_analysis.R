...
parser <- argparse::ArgumentParser(description=":: Perform parallel analysis of a Genomic SEM phenotype set ::", formatter_class="argparse.ArgumentDefaultsHelpFormatter")
...
wd <- paste0(args$outDir, args$name)
dir.create(file.path(wd), showWarnings = FALSE)
setwd(wd)

ldscout <- loadRData(file=args$rdata)

sink(paste0(wd, '/', args$name, 'parallel_analysis.log'))

paLDSC(S_Stand = ldscout$S_Stand, 
        V_Stand = ldscout$V_Stand, 
        r = 1000, 
        p = .95, 
        diag = T, 
        fa = T, 
